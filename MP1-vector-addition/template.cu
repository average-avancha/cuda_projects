...
__global__ void vecAdd(float *in1, float *in2, float *out, int len) {
  ...
  /*
   * (Device/GPU) Parallel Execution Code:  
   */

  //Index for the current thread
  ...
  /*
   *
   * (Host/CPU) Serial Execution Code for reference:
   * 
   * int i;
   * for(i = 0; i < len; ++i){
   *   out[i] = in1[i] + in2[i];
...
int main(int argc, char **argv) {
  ...
  // dim3 DIM_GRID(ceil(inputLength / 256.0), 1, 1);
  // dim3 DIM_BLOCK(256, 1, 1);

  int DIM_GRID = (int) ceil(inputLength / 256.0);
  int DIM_BLOCK = 256;

  wbTime_start(Compute, "Performing CUDA computation");
  //@@ Launch the GPU Kernel here
  vecAdd<<<DIM_GRID, DIM_BLOCK>>>(deviceInput1, deviceInput2, deviceOutput, inputLength);
