...
#define BLOCK_WIDTH 16.0 // floor( sqrt(NUM_THREADS) )
#define NUM_THREADS 256.0
// #define TILE_WIDTH 8.0
...
__global__ void matrixMultiplyShared(float *A, float *B, float *C,
  ...
  const size_t TILE_WIDTH = BLOCK_WIDTH;
  ...
  for (int q = 0; q < ceil((float)numAColumns/TILE_WIDTH); ++q) {
    ...
    for (int k = 0; k < TILE_WIDTH; ++k) {
      partial_sum += subTileA[ty][k] * subTileB[k][tx];
    }
  ...
  if ((Row < numCRows) && (Col < numCColumns)) {
    C[Row * numCColumns + Col] = partial_sum;
  }
}
...
int main(int argc, char **argv) {
  ...
  // printf("numCRows: %d\n", numCRows);
  // printf("numCRows: %d\n", numCColumns);
  // printf("BLOCK_WIDTH: %f\n", BLOCK_WIDTH);
  // printf("gridRows: %f\n", gridRows);
  // printf("gridColumns: %f\n", gridColumns);

  dim3 DimGrid(gridColumns, gridRows, 1);
