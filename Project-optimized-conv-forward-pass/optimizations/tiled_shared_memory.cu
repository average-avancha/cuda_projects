...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    */
    ...
    for(c = 0; c < C; c++){ 
        //Load tiled shared memory
        if((h < H) && (w < W)){
            in_shared[threadIdx.y][threadIdx.x] = in_4d(b, c, h, w);
        } else{
        ...
        for(p = 0; p < K; p++){
            for(q = 0; q < K; q++){
                if((h * S + p < H) && (w * S + q < W) && (threadIdx.y * S + p < BLOCK_WIDTH) && (threadIdx.x * S + q < BLOCK_WIDTH)){
                    // pConv += in_4d(b, c, h * S + p, w * S + q) * mask_4d(m, c, p, q);
                    pConv += in_shared[threadIdx.y * S + p][threadIdx.x * S + q] * mask_4d(m, c, p, q);
                }
                else if((h * S + p < H) && (w * S + q < W)){
                    pConv += in_4d(b, c, h * S + p, w * S + q) * mask_4d(m, c, p, q);
                }
            }
        }
